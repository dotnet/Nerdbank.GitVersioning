...
async function installNuGetPackageImpl(packageId: string, version: string): Promise<INuGetPackageInstallResult> {
    ...
    if (!(await existsAsync(packageLocation))) {
        ...
        let cmdline = `${nugetExePath} install ${packageId} -OutputDirectory ${packagesFolder} -Version ${version} -NonInteractive -Source https://ci.appveyor.com/nuget/nerdbank-gitversioning`;
        try {
            try {
